...
public class PrayTime {
    ...
    // ---------------------- Global Variables --------------------
    private Integer calcMethod; // caculation method
    ...
    private Hashtable methodParams;
    ...
    public PrayTime() {
        ...
        this.setCalcMethod(Integer.valueOf(0+""));
        ...
        this.setCustom(Integer.valueOf(""+7)); // Custom Setting
        ...
        // Time Names
        timeNames = new Vector();
        timeNames.addElement("Fajr");
        timeNames.addElement("Sunrise");
        timeNames.addElement("Dhuhr");
        timeNames.addElement("Asr");
        timeNames.addElement("Sunset");
        timeNames.addElement("Maghrib");
        timeNames.addElement("Isha");
...
//    }
    ...
    private double calcJD(int year, int month, int day) {
        ...
        double ms = calendar.getTime().getTime(); // # of milliseconds since midnight Jan 1,
        // 1970
    ...
    public Vector getDatePrayerTimes(int year, int month, int day,
            double latitude, double longitude, double tZone) {
    	System.out.println("year="+year+" month="+month+"day="+day+" lat="+latitude+" lng="+longitude+" tzone="+tZone);
    ...
    // return prayer times for a given date
    public Vector getPrayerTimes(Calendar date, double latitude,
    ...
    private void setCustomParams(double[] params) {
        ...
        for (int i = 0; i < 5; i++) {
            if (params[i] == -1) {
                params[i] = ((double[])methodParams.get(this.getCalcMethod()))[i];
                methodParams.put(this.getCustom(), params);
            } else {
            	((double[])methodParams.get(this.getCustom()))[i] = params[i];
            }
        }
    ...
    public String floatToTime24(double time) {
        ...
        if ((hours >= 0 && hours <= 9) && (minutes >= 0 && minutes <= 9)) {
            result = "0" + hours + ":0" + mMath.round(minutes);
        } else if ((hours >= 0 && hours <= 9)) {
            result = "0" + hours + ":" + mMath.round(minutes);
        } else if ((minutes >= 0 && minutes <= 9)) {
            result = hours + ":0" + mMath.round(minutes);
        } else {
            result = hours + ":" + mMath.round(minutes);
        }
        return result;
    }
    ...
    public String floatToTime12(double time, boolean noSuffix) {
        ...
        if (noSuffix == false) {
            if ((hours >= 0 && hours <= 9) && (minutes >= 0 && minutes <= 9)) {
                ...
                result = "0" + hours + ":" + mMath.round(minutes) + " " + suffix;
            } else if ((minutes >= 0 && minutes <= 9)) {
                result = hours + ":0" + mMath.round(minutes) + " " + suffix;
            } else {
                result = hours + ":" + mMath.round(minutes) + " " + suffix;
            }
    ...
    private double[] computeTimes(double[] times) {
        ...
        double Fajr = this.computeTime(
                180 - ((double[])methodParams.get(this.getCalcMethod()))[0], t[0]);
        ...
        double Maghrib = this.computeTime(
        		((double[])methodParams.get(this.getCalcMethod()))[2], t[5]);
        double Isha = this.computeTime(
        		((double[])methodParams.get(this.getCalcMethod()))[4], t[6]);
    ...
    // compute prayer times at given julian date
    private Vector computeDayTimes() {
    ...
    private double[] adjustTimes(double[] times) {
        ...
        if (((double[])methodParams.get(this.getCalcMethod()))[1] == 1) // Maghrib
        {
            times[5] = times[4] + ((double[])methodParams.get(this.getCalcMethod()))[2]/ 60;
        }
        if (((double[])methodParams.get(this.getCalcMethod()))[3] == 1) // Isha
        {
            times[6] = times[5] + ((double[])methodParams.get(this.getCalcMethod()))[4]/ 60;
        }
    ...
    private Vector adjustTimesFormat(double[] times) {

        Vector result = new Vector();

        if (this.getTimeFormat() == this.getFloating()) {
            for(int i=0;i<times.length;i++){
                result.addElement(String.valueOf(times[i]));
            }
            return result;
        }
        ...
        for (int i = 0; i < 7; i++) {
            if (this.getTimeFormat() == this.getTime12()) {
                result.addElement(floatToTime12(times[i], false));
            } else if (this.getTimeFormat() == this.getTime12NS()) {
                result.addElement(floatToTime12(times[i], true));
            } else {
                result.addElement(floatToTime24(times[i]));
            }
        }
        return result;
    }
    ...
    private double[] adjustHighLatTimes(double[] times) {
        ...
        // Adjust Fajr
        double FajrDiff = nightPortion(((double[])methodParams.get(this.getCalcMethod()))[0]) * nightTime;
        ...
        // Adjust Isha
        double IshaAngle = (((double[])methodParams.get(this.getCalcMethod()))[3] == 0) ? ((double[])methodParams.get(this.getCalcMethod()))[4] : 18;
        ...
        // Adjust Maghrib
        double MaghribAngle = (((double[])methodParams.get(this.getCalcMethod()))[1] == 0) ? ((double[])methodParams.get(this.getCalcMethod()))[2] : 4;
...
//        cal.setTime(now);
//
//        Vector prayerTimes = prayers.getPrayerTimes(cal,
//                latitude, longitude, timezone);
//        Vector prayerNames = prayers.getTimeNames();
//
//        for (int i = 0; i < prayerTimes.size(); i++) {
//            System.out.println(prayerNames.elementAt(i) + " - " + prayerTimes.elementAt(i));
//        }
//
//    }
    ...
    private Integer getCustom() {
        return Custom;
    }

    private void setCustom(Integer custom) {
        Custom = custom;
    }
